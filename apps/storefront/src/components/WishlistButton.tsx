...
export function WishlistButton({ productId, className }: { productId?: string; className?: string }) {
  ...
  if (productId) return <button type="button" aria-label={isSaved ? "Remove from wishlist" : "Add to wishlist"} aria-pressed={isSaved} onClick={() => toggle(productId)} className={cn("absolute right-3 top-3 z-20 flex size-9 items-center justify-center rounded-full  text-gray-900 transition-transform hover:scale-105", className)}><HeartIcon filled={isSaved} /></button>;
